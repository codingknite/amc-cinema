...
type Cinema = string;
type StartDate = Date | null | undefined;
...
const MakeBooking = (): JSX.Element => {
  const movieId: string = useParams().movieId;

  //global component state
  const [formsFilled, setFormsFilled] = useState<FormState>({
    session: false,
    tickets: false,
  });
  const [activeStep, setActiveStep] = useState<BookingState>({
    step1: true,
    step2: false,
    step3: false,
  });
  ...
  const handleStartTime = (_screen: string, _time: string) => {
    setStartTime(() => {
